...
export default function AuthPage({ initialMode = 'login' }) {
  const { login, signup, sendForgotPasswordOtp, resetPassword, isAuthenticated } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();

  const isSignupRoute = location.pathname === '/signup' || initialMode === 'signup';
  const isForgotRoute = location.pathname === '/forgot-password' || initialMode === 'forgot-password';
  const [mode, setMode] = useState(
    isSignupRoute ? 'signup' : isForgotRoute ? 'forgot-password' : 'login'
  );
            ...
            forgotStep === 1 ? (
              ...
              <>
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full mt-2 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-cyan-500 hover:from-emerald-500 hover:to-cyan-400 text-white font-bold shadow-lg shadow-cyan-500/25 flex items-center justify-center gap-2 transition-all cursor-pointer text-xs uppercase tracking-wider disabled:opacity-60"
                >
                  {isLoading ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
